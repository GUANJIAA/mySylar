...
void run()
{
    sylar::Address::ptr addr = sylar::Address::LookupIPAdress("www.baidu.com:80");
    if (!addr)
    {
    ...
    std::ofstream pfs("rsp.dat");
    pfs << *rsp;

    SYLAR_LOG_INFO(g_logger) << "============================";

    auto r = sylar::http::HttpConnection::DoGet("http://www.sylar.top/", 300);
