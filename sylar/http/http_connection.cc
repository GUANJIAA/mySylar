...
namespace sylar
{
    namespace http
    {
        ...
        {
            HttpResponParser::ptr parser(new HttpResponParser);
                                                  ...
                                                  uint64_t timeout_ms)
        {
            ...
            if (rt < 0)
            {
                return std::make_shared<HttpResult>((int)HttpResult::Error::SEND_SOCKET_ERROR,
                                                    nullptr, "send request socket error errno= " + std::to_string(errno) + " errstr=" + std::string(strerror(errno)));
            }
            auto rsp = conn->recvResponse();
            if (!rsp)
            {
                return std::make_shared<HttpResult>((int)HttpResult::Error::TIMEOUT,
                                                    nullptr, "recv response timeout: " + addr->toString() + " timeout_ms:" + std::to_string(timeout_ms));
            }
        ...
        HttpConnection::ptr HttpConnectionPool::getConnection()
        {
            ...
            if (!ptr)
            {
                IPAddress::ptr addr = Address::LookupIPAdress(m_host);
                if (!addr)
                {
                                                      ...
                                                      uint64_t timeout_ms)
        {
            ...
            {
                return std::make_shared<HttpResult>((int)HttpResult::Error::TIMEOUT,
                                                    nullptr, "recv response timeout: " + sock->getRemoteAddress()->toString() + " timeout_ms:" + std::to_string(timeout_ms));
            }
            return std::make_shared<HttpResult>((int)HttpResult::Error::OK, rsp, "ok");
        }
