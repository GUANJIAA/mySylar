...
namespace sylar
{
    ...
    Address::ptr Address::Create(const sockaddr *addr, socklen_t addrlen)
    {
        ...
        Address::ptr result;
        // SYLAR_LOG_INFO(g_logger) << addr->sa_family;
    ...
    IPAddress::ptr Address::LookupIPAdress(const std::string &host,
                                           int family, int type, int protocol)
    {
        std::vector<Address::ptr> result;
        if (Lookup(result, host, family, type, protocol))
                                        ...
                                        int family)
    {
        ...
        try
        {
            ...
            {
                // SYLAR_LOG_INFO(g_logger) << next->ifa_addr->sa_family;
                Address::ptr addr;
    ...
    IPAddress::ptr IPv4Address::subnetMask(uint32_t perfix_len)
    {
        ...
        subnet.sin_addr.s_addr = byteswapOnLittleEndian(CreateMask<uint32_t>(perfix_len));
    ...
    void IPv6Address::setPort(uint16_t val)
    {
